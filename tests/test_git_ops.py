...
from wharf.git_ops import push_refspec, push_revision, push_url
...
def test_push_refspec_is_forced_so_an_older_revision_can_be_deployed():
    # Without the "+", `deploy --revision <ancestor sha>` is rejected as a
    # non-fast-forward update to the bare repo's branch.
    assert push_refspec("abc123", "main") == "+abc123:refs/heads/main"
