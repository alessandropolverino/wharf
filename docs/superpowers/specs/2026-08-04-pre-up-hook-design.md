...
Verified against the real files (`compose/docker-compose.core.prod.yml`, `services/core/postgres/init/02_bootstrap_dashboard_admin.sh`) that the duplicate `bootstrap-dashboard-admin` entry is intentional, not a copy-paste bug: the script checks `to_regclass('public.users')` and exits 0 (no-op) if the dashboard schema doesn't exist yet, so the first call (before `migrate-janusdashboard`) is a deliberate safe no-op and the second call (after) does the real admin-user upsert. The follow-up task must keep both calls, in that order, not deduplicate them. Also confirmed none of `core`'s migration/bootstrap services have a `build:` section (they're public images with bind-mounted SQL/scripts) — `--build` is a no-op for this specific unit, but remains correct as a general default for `pre_up` consumers whose services do build custom images.
