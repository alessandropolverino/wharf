...
The URL comes from **`push_url(target, remote_repo)`** —
`ssh://user@host:port/path`, built from [`Target.address`](config.md),
which brackets an IPv6 host (`ssh://deploy@[2001:db8::1]:22/...`).
Unbracketed, git can't tell where the address ends and the port begins,
and hands ssh a mangled destination with no `-p`.
