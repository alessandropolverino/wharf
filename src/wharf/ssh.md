...
`SessionAuth.resolve(force_ci=None)` picks the mode: `force_ci` (the
CLI's `--ci`/`--interactive` flags) overrides autodetection via
[`is_ci()`](#is_ci), which checks the standard `CI` env var GitHub
Actions and most other CI systems set automatically.
