...
before every command except `ls` and `identities`.
...
limited, no releases published yet — is caught by a broad `except` and
treated identically to "nothing to report": an update notice must never
break a deploy. The request is unauthenticated (the repo is public), so
GitHub's 60-requests-per-hour unauthenticated limit applies; exceeding
it is just another silent failure.
