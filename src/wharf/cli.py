...
Every subcommand except ``ls`` accepts ``--ci``/``--interactive`` to
override wharf's automatic CI-vs-local detection (see wharf.ssh.is_ci).
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "deploy":
        ...
        return _run_operation(
            ...
            repo=repo, revision=revision, only=tuple(args.only), force_ci=force_ci,
        )

    if args.command == "down":
        return _run_operation(
            operations.down, config,
            repo=repo, only=tuple(args.only), volumes=args.volumes, force_ci=force_ci,
        )

    if args.command == "reload":
        return _run_operation(
            operations.reload, config,
            repo=repo, only=tuple(args.only), force_ci=force_ci,
        )
